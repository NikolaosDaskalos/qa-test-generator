...
**Owner Decision**:
The owner's human-in-the-loop verdict on an escalated Test Patch, supplied by resuming the suspended Coding Run. It is one of exactly three verdicts — Approval, rejection, or an Edit — never a binary accept/discard. The same suspended decision point produces all three; the owner never reaches a separate endpoint per verdict.
_Avoid_: Binary approve/reject, a separate endpoint per verdict, a model-decided verdict

**Edit**:
The owner's third Owner Decision verdict: instead of approving or rejecting the escalated Test Patch, the owner submits free-text feedback that returns the same Coding Run to generation, which **revises the already-generated Test Files in place** according to that feedback — the feedback is written against what was generated, so the model acts on the existing files and diff, not a blank slate — then re-reviews and re-escalates. The owner never hand-edits the Test Files; the model authors every revision. Feedback **accumulates** across successive Edits (each round's note is added to the prior ones, on top of the original Code Generation Task). An Edit **resets the automatic Generation Retries budget to zero**, giving the revised patch a fresh set of below-threshold retries, but keeps the run's generation branch, plan, and gathered Repository Documents — it does not re-plan, re-retrieve, discard the branch, or start a new Coding Run. It carries no dedicated run status: the resumed run cycles back through the ordinary generating and reviewing states and re-escalates for a fresh Owner Decision. Edit is unbounded — every round re-escalates and re-pauses, so a human gates each one — unlike the bounded, automatic Generation Retries.
_Avoid_: Owner-edited Test Files, from-scratch regeneration that ignores the generated files, discarding the generation branch, a new Coding Run, re-planning/re-retrieval, conflating it with a model-driven Generation Retry, a bounded/automatic loop, a dedicated status of its own
...
**AI Cost**:
An *estimated* monetary figure for the AI work a turn consumes, derived by multiplying provider-reported token usage by a locally maintained per-model price table — never a billed or provider-reported amount, since the providers used return token counts only, not money. It is attributed to the owning user, the Repository, the Repository Session, and the individual turn (a Session History message for a Repository question, or the Coding Run for a Code Generation Task), and is captured per model call so the per-turn, per-session, per-Repository, and per-user figures are all sums of the same underlying records — including the calls of a turn that later fails. It is read off the persisted records, never carried on the Agent Stream. Initially it covers chat-model calls only; embedding and rerank spend are out of scope for now, with room left to add them.
_Avoid_: Billed amount, invoice total, provider-reported cost, a gateway/proxy cost field, cost emitted on the Agent Stream, dropping the cost of a failed turn
