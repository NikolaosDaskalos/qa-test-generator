"""The shared context renderers: source-labeled evidence and proposed file blocks.
...
def test_format_evidence_labels_a_document_with_its_source() -> None:
    """A single document renders as a [Source: <path>] label above its content."""
    rendered = format_evidence([_source("app/auth.py", "def login(): ...")])
...
def test_format_evidence_falls_back_to_a_question_mark_when_source_is_missing() -> None:
    """A document with no ``source`` metadata is labeled ``[Source: ?]`` rather than erroring."""
    document = SourceDocument(content="orphan chunk", doc_metadata={})

    rendered = format_evidence([document])

    assert rendered == "[Source: ?]\norphan chunk"


def test_format_evidence_renders_an_empty_list_as_an_empty_string() -> None:
    """No evidence renders as the empty string so the caller can omit the section."""
