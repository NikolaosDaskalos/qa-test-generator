...
`text/event-stream` and is the only way to ask a Repository question, start a Test-Generation Task,
...
`run_rejected`). The same generator serves questions, test generation, and the approve/reject
